...
public class UserServices {
    ...
    public UserServices() {
        this.endPoint = "http://localhost:8091/api/user";
    ...
    public UserDTO login(LoginDTO loginDTO) {
        WebTarget target = client.target(endPoint + "/login");
        try {
            // Hacer la solicitud POST y obtener la respuesta
            Response response = target.request(MediaType.APPLICATION_JSON).post(Entity.entity(loginDTO, MediaType.APPLICATION_JSON));
            // Comprobar el estado HTTP de la respuesta
            if (response.getStatus() == Response.Status.OK.getStatusCode()) {
                return response.readEntity(UserDTO.class); // Leer y devolver el usuario
            } else {
            ...
            System.err.println("Excepción al iniciar sesión: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
